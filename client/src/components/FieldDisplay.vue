...
<template>
  <v-container id='contain' fluid>
    <Timeline v-if="trackingData" id="timeline"></Timeline>
    <div id="plotter"></div>
  </v-container>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    clearRoutes(){
      for(let i = 0; i < this.aframes.length; i++){
        this.aframes[i].data.pop()
      }
      },
    addRoutes(){
      ...
      // push extra marker on to field lngth = maxTime
      for(let i = 0; i < this.aframes.length -1; i+=10){

        //loop through playertracking data length = 20
        for(let j of this.trackingData.playertrackingdata){
          if(receivers.includes(j.playerid)){
            xvals.push(j.playertracking[i].x)
            yvals.push(j.playertracking[i].y)
          }
          // console.log('outerLoop:', i, 'innerLoop:', j, xvals, yvals)
      }
      
      console.log( xvals, yvals)
        // push a new marker with offensive x and y for each frame
        
      //   this.aframes[i].data.push(
      //     {mode: 'markers+text',
      //     name: 'receiver tracking',
      //     hoverinfo: 'none',
      //     x: xvals,
      //     y: yvals,
      //     opacity: .5,
    ...
    displayHandler(val){
      ...
      if(val.field.qbView === false){
        Plotly.restyle('plotter', hide, 4)
      } else {
        Plotly.restyle('plotter', show, 4)
      }
      // off play
      if(val.field.offPlay === false){
        Plotly.restyle('plotter', hide, 5)
      } else {
        Plotly.restyle('plotter', show, 5)
      }
      }
...
///////////////////////////////////////////////////
  ...
  this.triangleMachine()
    ...
    // set default to jerseys in store
    this.$store.commit('setJerseyDefault')
    console.log(this.aframes)
  }
}
</script>
